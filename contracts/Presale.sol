...
contract DawaPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount BUSD deposited by user
        uint256 debt; // total DAWA claimed
        bool claimed; // True if a user has claimed DAWA
    }

    address public MultiSignAddress = 0xa34130DC361b374d038916a3D9d178c9A8C8a561; 
    IERC20 public BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // for user deposits
    IERC20 public DAWA;

    uint256 public price = 2 * 1e18; // 2 BUSD per DAWA
    uint256 public cap = 1000 * 1e18; // 1000 BUSD cap per wallet
    uint256 public totalRaisedBUSD; // total BUSD raised by sale
    uint256 public totalDebt; // DAWA owed to users
    ...
    /**
     *  @notice set DAWA Token
     *  @param _DAWA: address of DAWA
     */
    function setDAWA(address _DAWA) external onlyOwner {
        require( _DAWA != address(0) );
        DAWA = IERC20(_DAWA);
    }

    /**
     *  @notice set price for DAWA
     *  @param _price: price for DAWA
     */
    ...
    // @notice lets users claim DAWA
    // @dev send sufficient DAWA before calling
    function claimUnlock() external onlyOwner {
        require(ended, "Sale has not ended");
        require(!claimable, "Claim has already been unlocked");
        require(DAWA.balanceOf(address(this)) >= totalDebt, 'not enough DAWA in contract');
        claimable = true;
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        BUSD.safeTransferFrom( msg.sender, MultiSignAddress, _amount);
        emit Deposit(msg.sender, _amount);
    }
    
    /**
     *  @notice claim DAWA from the presale contract
     */
    function claim() external checkIfPaused {
        require(claimable, 'DAWA is not yet claimable');
        UserInfo storage user = userInfo[msg.sender];
        require(!user.claimed, 'User already claimed DAWA');
        require(user.debt > 0, 'User should have unclaimed DAWA');
        require(DAWA.balanceOf(address(this)) >= user.debt, 'Not enough DAWA tokens on contract');
        uint256 amount = user.debt;
        DAWA.safeTransfer( msg.sender, amount );
        user.debt = 0;
        user.claimed = true;
        totalDebt = totalDebt.sub(amount);

        emit Withdraw(address(DAWA), msg.sender, amount);
    }
