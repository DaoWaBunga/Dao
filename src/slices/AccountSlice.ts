...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    const started = await presaleContract.started();
    const ended = await presaleContract.ended();
    const capable = await presaleContract.getUserRemainingAllocation(address);

    const claimable = await presaleContract.claimable();
    const userInfo = await presaleContract.userInfo(address);
